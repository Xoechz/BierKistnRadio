...
11. **Mute lives in `BluetoothClient.setMuted(bool)`**, not `VolumeController`: it is a property of the *BT stream's* node. Mechanism: `pw-dump`/`pw-cli ls Node` to find the `bluez_output.*.a2dp-sink` node ID, then `wpctl set-mute <id>`. Node discovery happens at call time (IDs are session-scoped).
