...
The Bluetooth sink is a **phone-driven** device: the phone finds, pairs, and connects to the Pi; the Pi advertises itself but never initiates pairing, discovery, or connection. The **connection UI is state-only** — it shows the currently-connected device and mediates a single takeover dialog; it does not enumerate devices or manage pairings. This ADR governs the *connection* model only; best-effort AVRCP *playback* controls (transport, metadata) are a separate layer that does not reverse phone-driven connection — see [ADR 0006](./0006-source-clients-and-best-effort-avrcp-controls.md). The Pi's discoverability/pair-ability is owned by the NixOS system config, not the app.
...
2. **Connection UI is state-only.** The Settings → Bluetooth module shows only the current connection state (and acts on takeover). No device list, no Pair/Connect/Disconnect rows. This is a *connection* boundary: best-effort AVRCP *playback* controls (Play/Pause/Next, metadata) may still be offered in Now-Playing when a device is connected — see [ADR 0006](./0006-source-clients-and-best-effort-avrcp-controls.md). Phone-driven connection and speaker-initiated playback control are orthogonal; the phone remains the initiator of pairing, connection, and playback.
