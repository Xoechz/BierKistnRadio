...
- [ ] **T26: Screen brightness controller.** Currently the RightSidebar brightness slider is pure UI state with no backend (see [SYSTEM_INTERFACE.md](./SYSTEM_INTERFACE.md) §9/§14.2 — the 7" panel is HDMI-driven, so no conventional `/sys/class/backlight` node exists; the interface is still open). Implement a `BrightnessController` (QML singleton, mirroring `VolumeController`) that reads/writes brightness through whatever interface the system repo provides — `wlr-brightness` (cage/Wayland), `ddcutil`, or a custom D-Bus backlight service — and wire the sidebar slider (`BrightnessController.brightness`) to it. Surface a visible "brightness unavailable" state rather than a silent failure, and keep the slider working in the UI when no backend is present.
  - Learn: backlight interfaces on HDMI panels (case-sensitive `wlr-randr`/`cage` backlight, `ddcutil`, D-Bus `org.freedesktop.login1`), clamps 0–100, read-back vs. write feedback-loop care (as in `VolumeController`).
