...
- [ ] **T3: PlaybackController — Bluetooth Mode (opt-in) + A2DP detection.** Bluetooth Mode is NOT triggered automatically by a BT A2DP source connecting. It is only entered via `switchToBluetooth()`:
  - `switchToBluetooth()`: pauses spotifyd (MPRIS2 `Pause`), then checks if an A2DP transport exists. If yes → `BluetoothActive` with `pairedDeviceName` from BlueZ. If no → calls `BluetoothController.setDiscoverable(true)`, sets `BluetoothWaiting`.
